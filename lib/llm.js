 ...
 * @param {string} [opts.effort]      'low' | 'medium' | 'high' reasoning depth
...
export async function complete(opts = {}) {
  ...
  // max_tokens is the ceiling on EVERYTHING the model emits, and on the current
  // models that includes the thinking it does before it writes. Set it too low
  // and the model reasons its way to the ceiling and returns a reply with no
  // text in it at all: stop_reason "max_tokens", one thinking block, no answer.
  // That is not a rare edge, it is a coin flip on every request. Give it room.
  const body = {
  ...
  // These tools write to a fixed structure from notes the manager already wrote.
  // That is not work that needs deep reasoning, and every second of thinking is a
  // second a manager sits watching a spinner. Ask for shallow thinking where the
  // caller says it is safe.
  ...
  // An empty completion used to come back as a success with no words in it.
  // Every caller then rendered nothing: no error, no spinner, no explanation,
  // and a 200 in the log saying all was well. A model that returns nothing has
  // failed, and it should say so here rather than three layers up.
  if (!text) {
    console.error('Anthropic returned no text', JSON.stringify({
      model: body.model,
      stop_reason: data && data.stop_reason,
      blocks: ((data && data.content) || []).map((b) => b && b.type),
      usage: data && data.usage,
    }));
    return { ok: false, text: '', status: 502, error: 'The model returned an empty answer. Please try again.' };
  }
